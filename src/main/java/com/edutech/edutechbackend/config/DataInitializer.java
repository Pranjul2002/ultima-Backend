 ...
 *
 * Step 1 — Wipe ALL previously seeded free tests.
 *           Uses TransactionTemplate so the transaction is opened
 *           programmatically — no proxy / self-invocation issues.
 *
...
public class DataInitializer implements CommandLineRunner {
    ...
    private final TransactionTemplate   transactionTemplate;   // ← injected, no proxy needed
    ...
    // ── Delete every free test owned by seed-mentor accounts ─────────────────
    // TransactionTemplate.executeWithoutResult() opens a real transaction
    // programmatically — bypasses the proxy/self-invocation limitation entirely.
            ...
            userRepository.findByEmail(email).ifPresent(mentor -> {
                ...
                if (!owned.isEmpty()) {
                    transactionTemplate.executeWithoutResult(status -> {
                        ...
                        testRepository.deleteAll(owned);
                    });
                    log.info("DataInitializer: deleted {} tests owned by {}.", owned.size(), email);
                }
            });
        }
    }
    ...
    private void seedTest(User mentor, String title, List<QuestionSeed> seeds) {
        ...
        if (exists) {
            log.info("DataInitializer: '{}' already exists — skipping.", title);
            return;
        }
    ...
    private List<QuestionSeed> aptitudeQuestions() {
        return List.of(
                ...
                new QuestionSeed("If 8 workers complete a job in 6 days, how many days will 4 workers take?",
                        "8","10","12","14","C",4),
                new QuestionSeed("Simple interest on Rs.1000 at 5% per annum for 2 years is:",
                        "Rs.50","Rs.100","Rs.150","Rs.200","B",4),
                new QuestionSeed("What is 2³ × 2² equal to?",
                        "2⁴","2⁵","2⁶","2⁸","B",4),
                new QuestionSeed("A shopkeeper buys an item for Rs.200 and sells it for Rs.250. Profit % is:",
                        "20%","25%","30%","50%","B",4),
                ...
                new QuestionSeed("How many prime numbers are between 1 and 20?",
                        "6","7","8","9","C",4),
                new QuestionSeed("Area of a circle with radius 7 cm? (pi = 22/7)",
